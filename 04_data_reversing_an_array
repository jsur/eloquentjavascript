...
function reverseArrayInPlace(array) {
  for(i = 0; i < Math.floor(array.length)/2; i++) {
    ...
    var x = array[i];
    //Change value at position i to be its opposite from the other end of the array
    array[i] = array[array.length - 1 - i];
    //Replace the opposite value with the original value from position i
